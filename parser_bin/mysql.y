...
inputstring: stmt_list
  | sql_segment
  | sql_segment ';' stmt_list
...
sql_segment: select_after_where
  ;

select_after_where: val_list opt_groupby opt_having opt_orderby opt_limit opt_union
  ; 
...
/* regular functions */
expr: RISK_FUNCTION '(' opt_val_list ')' {  emit("CALL RISK_FUNCTION %d %s", $3, $1); }
	| NAME '(' opt_val_list ')' {  emit("CALL %d %s", $3, $1); }
   ;
...
expr: FSUBSTRING '(' val_list ')'               {  emit("CALL %d SUBSTR", $3); }
   ...
   | CONVERT '(' expr USING NAME ')' { emit("CALL 2 CONVERT"); }
   | CONVERT '(' expr ',' NAME ')' { emit("CALL 2 CONVERT"); }
   
   /*** add if ***/
   | IF '(' expr ',' expr ',' expr ')' { emit("IF") }
...
expr: CURRENT_TIMESTAMP { emit("NOW"); };
   ...
   | CURRENT_USER       { emit("CURRENT_USER") }
...
select_stmt: SELECT select_opts select_expr_list opt_where opt_groupby opt_having opt_orderby opt_limit opt_into_list opt_union
                        { emit("SELECTNODATA %d %d", $2, $3); }
    | SELECT select_opts select_expr_list  FROM table_references opt_where opt_groupby opt_having opt_orderby opt_limit opt_into_list opt_union { emit("SELECT %d %d %d", $2, $3, $5); }
   ;

opt_union: /* nil */
   | UNION select_opts select_stmt
   | UNION select_opts '(' select_stmt ')' opt_as_alias
   ;
...
opt_limit: /* nil */ | LIMIT expr { emit("LIMIT 1"); }
  ...
  | LIMIT expr OFFSET expr        { emit("LIMIT 2"); } 
  ; 
...
select_opts:                          { $$ = 0; }
| select_opts ALL                 
   { if($1 & 01) yyerror("duplicate ALL option"); $$ = $1 | 01; return 1;}
| select_opts DISTINCT            
   { if($1 & 02) yyerror("duplicate DISTINCT option"); $$ = $1 | 02; return 1;}
| select_opts DISTINCTROW         
   { if($1 & 04) yyerror("duplicate DISTINCTROW option"); $$ = $1 | 04; return 1;}
| select_opts HIGH_PRIORITY       
   { if($1 & 010) yyerror("duplicate HIGH_PRIORITY option"); $$ = $1 | 010; return 1;}
| select_opts STRAIGHT_JOIN       
   { if($1 & 020) yyerror("duplicate STRAIGHT_JOIN option"); $$ = $1 | 020; return 1;}
| select_opts SQL_SMALL_RESULT    
   { if($1 & 040) yyerror("duplicate SQL_SMALL_RESULT option"); $$ = $1 | 040; return 1;}
| select_opts SQL_BIG_RESULT      
   { if($1 & 0100) yyerror("duplicate SQL_BIG_RESULT option"); $$ = $1 | 0100; return 1;}
...
table_factor:
  ...
  | '`' NAME '`' opt_as_alias index_hint { emit("TABLE %s", $2); }
  | '`' NAME '.' NAME '`' opt_as_alias index_hint { emit("TABLE %s.%s", $2, $4); }                               
  | table_subquery opt_as NAME { emit("SUBQUERYAS %s", $3); }
  | '(' table_references ')' { emit("TABLEREFERENCES %d", $2); }
  | '{' NAME table_reference '}' { emit("TABLE ALIAS %s", $2); }
  ;
...
insert_stmt: INSERT insert_opts opt_into NAME
     ...
     opt_ondupupdate { emit("INSERTVALS %d %d %s", $2, $7, $4); }
   ;
...
insert_asgn_list:
     NAME COMPARISON expr 
       ...
       emit("ASSIGN %s", $1); $$ = 1; return 1;}
   | NAME COMPARISON DEFAULT
       { if ($2 != 4) { yyerror("bad insert assignment to %s", $1); YYERROR; }
                 emit("DEFAULT"); emit("ASSIGN %s", $1); $$ = 1; return 1;}
   | insert_asgn_list ',' NAME COMPARISON expr
       { if ($4 != 4) { yyerror("bad insert assignment to %s", $1); YYERROR; }
                 emit("ASSIGN %s", $3); $$ = $1 + 1; return 1;}
   | insert_asgn_list ',' NAME COMPARISON DEFAULT
       { if ($4 != 4) { yyerror("bad insert assignment to %s", $1); YYERROR; }
                 emit("DEFAULT"); emit("ASSIGN %s", $3); $$ = $1 + 1; return 1;}
   ;
...
column_atts: /* nil */ { $$ = 0; }
    ...
    | column_atts DEFAULT STRING        
        { emit("ATTR DEFAULT STRING %s", $3);  $$ = $1 + 1; }
